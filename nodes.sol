...
contract SkynetNode is ERC721Enumerable, Ownable, ControlledAccess{
    using Strings for uint256;
    string baseURI;
    ...
    uint[4] public nodeCount = [0,0,0,0];
    mapping(uint256 => uint) public nodeType;
    uint[4] public maxSupplyPerNode = [2500,2500,2500,2500];
    // tax in 1e4 (10000 = 100%)
    uint256 public tax;
    uint256[4] public mintPrices=[100000000, 1000000000, 5000000000, 10000000000];
    ...
    function getMintPrices() public view returns (uint256[4] memory) {
        return mintPrices;
    }

    function updateMaxSupplyPerNode(uint256[4] memory _newMaxSupplyPerNode) public onlyOwner {
        maxSupplyPerNode = _newMaxSupplyPerNode;
    }

    function updateMintPrices(uint256[4] memory _prices) public onlyOwner{
        mintPrices = _prices;
    }
    ...
    function updateSkynetAddress( address _skynetAddress) public onlyOwner{
        Skynet = _skynetAddress;
    }
    ...
    function _mint(uint _type, address to) internal{
        ...
        require(_type>0 && _type<=4, "Invalid node type");
        require(
    ...
    function mint(uint _type) public {
        ...
        if (msg.sender != owner()) {
            require(IERC20(Skynet).transferFrom(msg.sender, address(this), price), "Not enough funds");
            IERC20(Skynet).transfer(rewardPool, price-_tax);
        }
