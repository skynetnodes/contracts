...
contract SkynetERC20Token is ERC20Permit, Ownable {
    using SafeMath for uint256;
    ...
    constructor() 
    ERC20("Skynet", "Skynet", 9) 
    ERC20Permit("Skynet") 
    {
        initialSupply = 1000*1e9;
        ...
        if(limitsInEffect){
            if (
                ...
                if (transferDelayEnabled){
                    if (to != owner() && !_isExcludedMaxTransactionAmount[to] && !_isExcludedMaxTransactionAmount[from]){
                ...
                if (!_isExcludedMaxTransactionAmount[from] && enableBlock != 0 && block.number == enableBlock){
