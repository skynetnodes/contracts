...
contract rewardPool is Ownable {
    ...
    function addNodeInfo(uint256 _nftId, uint256 _nodeType, address _account) external onlyNodes returns (bool success) {
        ...
        nftInfo[_nftId].expiry = block.timestamp + (day*expiryDays[_nodeType-1]);
        totalNodes += 1;
        return true;
    }

    function addNodeInfoMigration(uint256 _nftId, uint256 _nodeType, address _account, uint256 _lastClaim) external onlyOwner returns (bool success) {
        require(nftInfo[_nftId].owner == address(0), "Node already exists");
        nftInfo[_nftId].nodeType = _nodeType;
        nftInfo[_nftId].owner = _account;
        nftInfo[_nftId].lastClaim = _lastClaim;
        nftInfo[_nftId].expiry = block.timestamp + (day*expiryDays[_nodeType-1]);
        totalNodes += 1;
        return true;
    }

    function updateUserInfoPostMigration(address _account, uint256 _lastClaim, uint256 _soldNodeRewards) external onlyOwner returns (bool success){
        lastClaimed[_account] = _lastClaim;
        soldNodeRewards[_account] = _soldNodeRewards;
        return true;
    }
